...
func (ks *eth) Disable(address common.Address, chainID *big.Int, qopts ...pg.QOpt) error {
	...
	return ks.disable(address, chainID)
}

func (ks *eth) disable(address common.Address, chainID *big.Int, qopts ...pg.QOpt) error {
	q := ks.orm.q.WithOpts(qopts...)
	_, err := q.Exec(`UPDATE evm_key_states SET disabled = true, updated_at = NOW() WHERE address = $1 AND evm_chain_id = $2`, address, chainID.String())
	if err != nil {
...
func (ks *eth) Reset(address common.Address, chainID *big.Int, nonce int64, qopts ...pg.QOpt) error {
	q := ks.orm.q.WithOpts(qopts...)
	res, err := q.Exec(`UPDATE evm_key_states SET next_nonce = $, updated_at = NOW() WHERE address = $2 AND evm_chain_id = $3`, nonce, address, chainID.String())
	if err != nil {

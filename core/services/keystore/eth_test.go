...
func Test_EthKeyStore_CheckEnabled(t *testing.T) {
	...
	t.Run("enabling a key that does not exist isn't allowed", func(t *testing.T) {
		addr := utils.RandomAddress()
		err := ks.Enable(addr, testutils.FixtureChainID)
		assert.EqualError(t, err, "no key exists with ID %s", addr.Hex())
	})
	t.Run("disabling a key that does not exist isn't allowed", func(t *testing.T) {
		addr := utils.RandomAddress()
		err := ks.Disable(addr, testutils.FixtureChainID)
		assert.EqualError(t, err, "no key exists with ID %s", addr.Hex())
	})
